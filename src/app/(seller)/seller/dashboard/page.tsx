    ...
    return (
      <div className="p-6 max-w-lg">
        <div className="card p-6 border-l-4 border-amber-400">
          <h2 className="font-bold text-lg text-ink-900 mb-2">Ombi lako linasubiri ukaguzi</h2>
    ...
    return (
      <div className="p-6 max-w-lg">
        <div className="card p-6 border-l-4 border-red-400">
          <h2 className="font-bold text-lg text-ink-900 mb-2">Duka lako limesimamishwa</h2>
          <p className="text-sm text-ink-600">Wasiliana na msimamizi kwa maelezo zaidi.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display font-black text-2xl text-ink-900">Habari, {seller?.store_name}</h1>
          <p className="text-sm text-ink-500 mt-0.5">Hizi ndizo takwimu za duka lako</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/seller/orders" className="text-xs text-brand-600 font-medium">Yote →</Link>
          </div>
            ...
            <div className="space-y-2">
              {analytics?.recentOrders.map((order: any) => (
                <Link key={order.id} href={`/seller/orders`}
                  className="flex items-center justify-between p-2.5 rounded-xl hover:bg-ink-50 transition-colors">
                  <div>
                    <p className="text-xs font-semibold text-ink-700">#{typeof order.id === 'string' ? order.id.slice(-8).toUpperCase() : order.id}</p>
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-ink-800">Bidhaa za Juu</h2>
            <Link href="/seller/products" className="text-xs text-brand-600 font-medium">Zote →</Link>
          </div>
            ...
            <div className="space-y-2">
              {analytics?.topProducts.map((p: any, i: number) => (
                <div key={p.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-ink-50">
                  <span className="text-xs font-black text-ink-400 w-4">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-ink-800 truncate">{p.name}</p>
                    <p className="text-xs text-ink-500">Mauzo: {p.total_sold}</p>
                  </div>
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${p.stock_quantity <= 5 ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
